...
- [x] `processor/valuation.py` — `available_period()`(시점 매핑), `market_caps()`, `pbr()`
- [x] **단위 오류 대응** — 값을 고치지 않고 못 믿는 종목을 뺀다. 두 겹으로 거른다:
      (1) 같은 종목 기간별 중앙값 대비 5배 밖 (2) 자본총계 대비 시총이 PBR 50 초과.
      (2)가 필요한 이유는 조선내화처럼 3개 기간 중 2개가 함께 틀리면 중앙값이 오염되기 때문
- [x] PBR 팩터 워크포워드 검증 → **기각**. 대조군(PBR 미적용)보다 나빴다.
      `research/fundamental_walkforward.py`, 결과는 `research/README.md` 일곱 번째 사례.
      전략 코드에는 반영하지 않는다
- [ ] **2021Q4 수집** — 2022년 시점의 시가총액은 2021 사업보고서(2022년 3월 공시) 값을
      써야 한다. 2022Q4를 2022년에 쓰면 1년 앞선 정보를 참조하게 된다
...
- [x] **E3. 중복 주문 방지** — `executor/guard.py:already_entered()`가 모의·실전 매수
      앞단에서 (1) 이미 보유 중 (2) 오늘 이미 매수함 을 막는다.
      `positions`는 `ON CONFLICT DO NOTHING`이었지만 `trades`에는 한 줄이 더 들어가
      매수금액이 두 번 잡히고 현금·자산 스냅샷이 틀어지던 문제
- [~] **E4. KIS 응답 필드 검증** — 잔고 조회는 정상 동작 확인(`rt_cd 0`, 예수금 1,000만원)
      했지만 **보유 종목이 0건이라 `output1` 필드명은 확인하지 못했다**.
      대신 `executor/live.py:_field()`가 필드가 없으면 실제 키 목록을 담아 즉시 알린다.
      확정하려면 모의계좌로 1주 매수해 보유를 만든 뒤 `live.get_balance()`를 호출할 것
