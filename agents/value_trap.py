"""
가치 함정 판별 에이전트 (거부권 없음 — 합의 대상)

싸게 보이는 종목 안에는 사이클 하단(사야 함)과 구조적 훼손(사면 안 됨)이 섞여
있다. 가격·재무 숫자로는 둘이 똑같이 생겼다 — 이익이 무너지는 중이면 PER은
오히려 낮게 찍힌다. 구별하는 정보는 공시 텍스트뿐이라 여기서만 LLM을 쓴다.

퀄리티/가치 전략(quality_v1)이 특히 이 위험에 노출된다. 2026-08-21 편입 종목이
PER 2.2~4.4배 · PBR 0.14~0.27배이고 부채비율이 167%까지 올라간다. 싼 데는
이유가 있을 수 있고, 그 이유는 재무제표 숫자보다 공시에 먼저 나타난다.

에이전트에는 후보 필터가 쓰지 않은 정보만 준다. 역발상에서는 heat_score와 그
구성 지표를, 퀄리티에서는 PER·PBR·ROE를 넘기지 않는다 — 필터가 이미 쓴 지표를
다시 물으면 판단이 아니라 동어반복이 된다 (research/README.md "LLM 게이트가
실제로는 아무 결정도 바꾸지 않았다" 참고).
"""
import sys, os
