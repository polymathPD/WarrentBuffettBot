...
import config
from collector.dart_corp_code import DartCorpCodeCollector
...
def test_parse_keeps_only_listed_companies():
    ...
    mapping = DartCorpCodeCollector().parse(SAMPLE_XML)
...
def test_fetch_xml_rejects_non_zip_response(mocker):
    ...
    mocker.patch("collector.dart_corp_code.requests.get", return_value=resp)

    with pytest.raises(RuntimeError, match="zip 응답이 아님"):
        DartCorpCodeCollector().fetch_xml()
...
def test_collect_updates_only_known_codes(mock_db, mocker):
    ...
    updated = DartCorpCodeCollector().run()
