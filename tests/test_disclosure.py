...
from collector import disclosure as dis


def test_month_ranges_splits_on_calendar_months():
    got = list(dis._month_ranges(date(2026, 1, 15), date(2026, 3, 3)))
...
def test_month_ranges_single_day():
    assert list(dis._month_ranges(date(2026, 8, 18), date(2026, 8, 18))) == [
...
def test_collect_stops_at_last_page(mock_db, mocker):
    ...
    mocker.patch.object(dis.config, "DART_API_KEY", "dummy")
    mock_db.fetchall.return_value = [{"code": "005930"}]
    mock_db.fetchone.return_value = None
    fetch = mocker.patch.object(dis, "fetch_page", return_value={
        "status": "000", "total_page": 1,
        "list": [{"stock_code": "005930", "rcept_no": "1",
                  "rcept_dt": "20260818", "report_nm": "보고서"}],
    })

    saved = dis.collect("2026-08-18", "2026-08-18")
...
def test_collect_skips_cursor_update_on_failure(mock_db, mocker):
    ...
    mock_db.fetchall.return_value = [{"code": "005930"}]
    mock_db.fetchone.return_value = None
    mocker.patch.object(dis, "fetch_page", side_effect=RuntimeError("DART 오류 800"))
...
def test_collect_treats_no_data_status_as_empty(mock_db, mocker):
    ...
    mock_db.fetchall.return_value = [{"code": "005930"}]
    mock_db.fetchone.return_value = None
    mocker.patch.object(dis, "fetch_page", return_value={"status": dis.NO_DATA})
...
def test_collect_without_api_key_is_a_noop(mock_db, mocker):
    mocker.patch.object(dis.config, "DART_API_KEY", "")

    assert dis.collect("2026-08-18", "2026-08-18") == 0
