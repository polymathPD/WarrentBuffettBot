...
**3단계 — 리밸런싱**: 전월 마지막 거래일 기준으로 점수 상위 `SLOTS`개를 골라
**그 달 첫 거래일 시가**에 동일가중으로 편입합니다(체결 규칙 = 기준일 다음 거래일 시가).
...
(실제 네트워크·DB 연결 없이 수 초 내 완료). **30개 파일 315건.**
