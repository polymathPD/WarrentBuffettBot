...
import config
from collector.base import Collector, dart_get, DART_NO_DATA
...
class DisclosureCollector(Collector):
    SOURCE = "disclosure"
    ...
    def fetch_page(self, bgn: date, end: date, corp_cls: str, page_no: int) -> dict:
    ...
    def rows(self, data: dict, known: set) -> list[tuple]:
    ...
    def last_cursor(self) -> date | None:
        row = db.fetchone(
    ...
    def run(self) -> int:
        ...
        for bgn, fin in _month_ranges(start, end):
            for corp_cls in self.CORP_CLASSES:
                page_no = 1
                while page_no <= self.MAX_PAGES:
                    try:
                        data = self.fetch_page(bgn, fin, corp_cls, page_no)
                    except Exception as e:
                        print(f"  [실패] {bgn}~{fin} {corp_cls} p{page_no}: {e}")
                        failed += 1
                        break

                    if data.get("status") == DART_NO_DATA:
                        break

                    rows = self.rows(data, known)
                    if rows:
                        db.executemany(
