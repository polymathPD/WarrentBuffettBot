...
def snapshot(target_date: str = None) -> int:
    ...
    # 오늘 것으로 복제된다 — valuation_job(10:00)과 open_job(10:30·13:10)이
    # 장중에 이 함수를 부르고, 그 시각에는 오늘 봉이 없다. 16:10 배치가 덮어쓰기
    # 전에 죽으면 그 복제본이 그대로 굳는다. 2026-08-25 paper 스냅샷이 08-24와
    # 총자산·평가금액까지 똑같았던 것이 그 경우다(실제 08-25 종가로는 9,929,300이었다).
    #
